...
class TestSubagentMiddleware:
    ...
    def test_defined_subagent(self):
        # Create weather subagent using OOP style
        weather_subagent = create_deep_agent(
            name="weather",
    ...
    def test_defined_subagent_custom_model(self):
        # Create weather subagent with custom model using OOP style
        weather_subagent = create_deep_agent(
            name="weather",
            description="This subagent can get weather in cities.",
            system_prompt="Use the get_weather tool to get the weather in a city.",
    ...
    def test_defined_subagent_custom_middleware(self):
        # Create weather subagent with custom middleware using OOP style
        weather_subagent = create_deep_agent(
            name="weather",
            description="This subagent can get weather in cities.",
            system_prompt="Use the get_weather tool to get the weather in a city.",
    ...
    def test_defined_subagent_custom_runnable(self):
        # Create custom weather subagent using OOP style
        # In OOP fork, we just pass the Agent object directly, no need to wrap in dict
        weather_subagent = create_deep_agent(
            name="weather",
            description="This subagent can get weather in cities.",
            system_prompt="Use the get_weather tool to get the weather in a city.",
    ...
    def test_multiple_subagents_with_interrupt_on_no_middleware_accumulation(self):
        # In OOP fork, each subagent is created independently with its own config
        # Test that multiple subagents can be created with interrupt_on without issues
        subagent1 = create_deep_agent(
            name="subagent1",
        ...
        # Verify both subagents are available
        assert "task" in agent.nodes["tools"].bound._tools_by_name.keys()
        # This would error if middleware was incorrectly accumulated
        assert True